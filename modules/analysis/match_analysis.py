...
def filter_jobs_by_salary(jobs: List[Dict], min_salary: Optional[float] = None,
    ...
    for job in jobs:
        ...
        job_min_sal, job_max_sal = extract_salary_from_text_regex(salary_str)
        
        if not job_min_sal:
            job_min_sal, job_max_sal = extract_salary_from_text_regex(description)
...
def calculate_salary_band(matched_jobs: List[Dict] = None, 
                          expected: float = None) -> Dict[str, float]:
